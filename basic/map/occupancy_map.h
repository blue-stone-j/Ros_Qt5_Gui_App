...
class OccupancyMap
{
 public:
  ...
  int rows{0};              // row(height)
  int cols{0};              // col(width)
 ...
 public:
  ...
  OccupancyMap(int rows_, int cols_, Eigen::Vector3d origin, double res, Eigen::MatrixXi data) :
  ...
  // 输入原始栅格地图数据(地图未翻转) input original grid map(unflipped)
  ...
  GetCostMapData()
  {
    Eigen::Matrix<Eigen::Vector4i, Eigen::Dynamic, Eigen::Dynamic> res =
        Eigen::Matrix<Eigen::Vector4i, Eigen::Dynamic, Eigen::Dynamic>(map_data.rows(), map_data.cols());
    for (int x = 0; x < map_data.rows(); x++)
    {
      for (int y = 0; y < map_data.cols(); y++)
      {
        ...
        res(x, y) = color_rgba;
      } // endfor: have traversed this x
    } // endfor: have traversed all
    return res;
  }

  // save map to specific path
  void Save(std::string map_name)
  {
    ...
    for (unsigned int y = 0; y < height(); y++)
    {
      ...
      {
        // set color according to occupied status
        // unsigned int i = x + (height() - y - 1) * map->info.width;
        if (map_data(y, x) >= 0 && map_data(y, x) <= map_config.free_thresh)
        { // [0,free)
          fputc(254, out);
        }
        else if (map_data(y, x) >= map_config.occupied_thresh)
        { // (occ,255]
          fputc(000, out);
        }
        else
  ...
  bool Load(const std::string &yaml_path)
  {
    ...
    unsigned char *p;
    unsigned char value;
    int rowstride, n_channels, avg_channels; // rgb channel
    unsigned int i, j;                       //???
    int k;                                   //???
    double occ;
    int alpha;
    ...
    for (j = 0; j < height; j++)
    {
      for (i = 0; i < width; i++)
      {
        ...
        {
          alpha = *(p + n_channels - 1); // take last channel as alpha
        }
        ...
        // If negate is true, we consider blacker pixels free, and other pixels occupied.  Otherwise, it's vice versa.
        occ = (255 - color_avg) / 255.0;

        // Apply thresholds to RGB means to determine occupancy values for map.  Note that we invert the
        // graphics-ordering of the pixels to produce a map with cell (0,0) in the lower-left corner.
        ...
        map_data(j, i) = value;
      } // endfor: have traversed this height
    } // endfor: have traversed all
